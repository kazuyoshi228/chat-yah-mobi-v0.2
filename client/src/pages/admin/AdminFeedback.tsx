...
export default function AdminFeedback() {
  const { data, isLoading } = trpc.admin.listFeedback.useQuery({ limit: 200 });

  const avgRating = data && data.length > 0
    ? (data.reduce((s, d) => s + d.rating, 0) / data.length).toFixed(1)
    : "—";
  const resolvedCount = data?.filter((d) => d.resolved === "yes").length ?? 0;
  const withComments = data?.filter((d) => d.freeComment || d.comment).length ?? 0;
