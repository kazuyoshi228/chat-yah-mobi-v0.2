...
export default function Refund() {
  ...
  const { data: incidentsData, isLoading } = trpc.plans.incidents.useQuery({
    limit: 100,
    offset: 0,
                ...
                <TableBody>
                  {incidents.map((inc) => (
                    <TableRow key={inc.id}>
                      ...
                      <TableCell>
                        <StatusBadge status={inc.refundStatus} />
                      </TableCell>
                      <TableCell className="text-xs text-muted-foreground whitespace-nowrap">
                        {inc.detectedAt
                          ? new Date(inc.detectedAt).toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" })
                          : "—"}
                      </TableCell>
                      <TableCell className="text-xs text-muted-foreground whitespace-nowrap">
                        {inc.resolvedAt
                          ? new Date(inc.resolvedAt).toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" })
                          : "—"}
                      </TableCell>
                    </TableRow>
                  ))}
