...
## Phase 46: 返金不可ポリシーの全面展開（多言語RAG・購入フロー・特商法ページ）
- [x] 多言語RAG（中国語・韓国語・タイ語・ベトナム語）に返金不可ポリシーセクションを追加（Phase 50で実施済み）
- [~] 購入フロー同意チェックボックスの文言を確認（yah.mobi本体 — このシステム外の確認事項）
- [~] 取扱説明書（日本語MD・日英PDF）に購入フロー同意チェックボックスのスクリーンショット反映（スクリーンショット未取得のため保留）
- [~] 特定商取引法ページ（LegalPage.tsx）の返金ポリシー記述を更新（この管理画面には存在しないため対象外）
- [x] 日英PDF再生成・ストレージアップロード・UserManuals.tsx URL更新（Phase 50で実施済み）

## Phase 47: チャットボット100%対応化（訪問者側のオペレーター引き継ぎ廃止）
- [x] AIシステムプロンプトを「3回未解決→お問い合わせフォーム誤導」ロジックに変更
- [x] オペレーターへの引き継ぎ指示をシステムプロンプトから削除（フォーム誤導に変更）
- [x] 訪問者側チャットUIの「Connect with an operator」ボタンを削除
...
## Phase 51: 会話履歴全件参照・AIサマリー確認
- [x] WidgetChat.tsxのヘッダー「AI Support」確認（対応済み）
...
## Phase 56: RAGドキュメント改善（インストール手順・料金プラン・返金ポリシー追加）
- [x] 取扱説明書の内容を確認し、RAGドキュメントの素材を準備
- [x] eSIMインストール手順書（iPhone/Android別・APN設定・トラブルシューティング含む）を6言語で作成・DB登録
- [x] 料金プラン詳細（6プラン・追加データ・支払方法）を6言語で作成・DB登録
- [x] 返金・キャンセルポリシー（法的根拠・例外ケース含む）を6言語で作成・DB登録
- [x] 取扱説明書の内容をRAGに統合（FAQ既存内容＋新規3種類で網羅）
- [x] シミュレーション再実行でRAGスコア改善を確認（0.324→0.391、+20.7%改善）
- [x] チェックポイント保存

## Phase 57: デシジョンツリー型チャットフロー実装
- [x] 質問カテゴリの出現確率・クレームリスク分析（シミュレーション結果・RAGドキュメント・FAQ傾向から）
- [x] デシジョンツリー設計（6言語対応・クレーム防止フロー含む）
- [x] DBスキーマ: chat_flow_nodes テーブル追加（ツリー定義をDB管理）
- [x] バックエンド: chat.getFlowNodes / chat.submitContactForm tRPCクエリ追加
- [x] チャットウィジェット: 開始時にカテゴリ選択ボタンを表示（接続/購入/返金/その他）
- [x] チャットウィジェット: 選択に応じて次の質問ボタンを表示（デシジョンツリー）
- [x] チャットウィジェット: リーフノードで解決策テキストを表示
...
## Phase 59: 100セッションシミュレーション
- [ ] 100シナリオ設計（6言語×多様なカテゴリ・エッジケース含む）
- [ ] シミュレーションスクリプトを100セッション対応に拡張
- [ ] 100セッション実行・DB保存
- [ ] 結果分析・弱点特定・改善提案レポート作成

## Phase 59: RAGスコア目標達成（接続0.8以上・料金0.9以上・返金0.9以上）
- [ ] 現在のRAGドキュメント内容を分析し、スコア低下の原因を特定
- [ ] RAGドキュメントを大幅強化（キーワード密度向上・詳細手順追加・6言語）
...
- [ ] RAGドキュメントを細分化（接続トラブルをiPhone/Android/インストール前後に分割）
...
- [x] searchRagDocumentsのtopKを3→5（通常）/7（flowContext時）に増加
- [x] ハイブリッド検索実装（embedding + N-gramキーワードマッチング）
- [x] デシジョンツリーflowContext情報を大幅拡充（pricing: 全プラン詳細・支払方法・推薦ロジック・購入フロー）
- [x] デシジョンツリーflowContext情報を大幅拡充（refund: 完全ポリシー・例外3ケース・対応フロー・よくあるシナリオ・プラン変更不可ポリシー）
...
## Phase 74: CLEANUP_REPORT改善提案の全実施

### セキュリティ強化
- [x] Helmet.js 導入（CSP, X-Frame-Options, HSTS等のHTTPセキュリティヘッダー）
- [x] 入力サニタイゼーション強化（XSSペイロード検出・除去 + スパム検出 + 無意味メッセージ検出）
- [x] Webhook HMAC-SHA256 署名検証の実装

### チャット悪用対策
- [x] IPベースのレート制限（100 req/10min per IP）
- [x] CAPTCHA統合（Cloudflare Turnstile サーバーサイド検証実装済み。フロントエンド統合は次フェーズ）
- [x] コンテンツフィルタリング（同一内容の反復送信ブロック）
- [x] 無意味メッセージ検出（ランダム文字列・意味のない入力をブロック）
- [x] コスト上限アラート（$50/日 + 超過時AI一時停止 + オーナー通知）

### ページ統合・ミニマル化
- [x] HistoricalDocs.tsx → SSoTに参考資料リンクとして統合し削除
...
## Phase 77: タイ語・ベトナム語RAGドキュメントの整備

- [x] 既存のタイ語・ベトナム語RAGドキュメントの現状確認（各16件）
- [x] 不足カテゴリを特定（APN iPhone/Android・サービス概要・購入ガイド・FAQ Top10・出発前チェックリスト・現地接続ガイド・デバイス別インストール・OONAS・返金ポリシー詳細）
