...
function buildFlowContextPrompt(flowContext) {
  ...
  if (flowContext.category === "connection") {
    ...
    lines.push("");
    lines.push("### Response Flow — CRITICAL: ALWAYS state no-refund policy FIRST, even for exception cases");
    lines.push("1. Acknowledge frustration empathetically");
    lines.push("2. **ALWAYS state no-refund policy AND legal basis FIRST** (even if exception applies)");
    lines.push("   Example: 'Our policy is that eSIM purchases are non-refundable once payment is complete, per Japan Act on Specified Commercial Transactions Article 15-3. You agreed to this at checkout.'");
    lines.push("3. THEN ask if situation matches an exception");
    lines.push("4. If exception: STILL state policy first, then 'However, your case may qualify as exception' and guide to contact form");
    lines.push("5. If not: explain clearly but kindly that refund cannot be processed, citing legal basis");
    lines.push("");
    ...
    lines.push("- 'Haven't used it' → State policy + legal basis. No refund (digital product delivered = QR sent)");
    lines.push("- 'Bought wrong plan' → State no-refund policy + legal basis. No plan switching either. Alternatives:");
    ...
    lines.push("- 'Doesn't work' → State policy first, then troubleshoot; if system failure, guide to form");
    lines.push("- 'Charged twice' → State policy first, then: 'However, duplicate charges are exception. Contact us via yah.mobi/app with payment details'");
    lines.push("- 'Didn't receive QR' → State policy first, then: 'However, if system failed to issue QR, this is exception. Check spam first; if not there, contact us via yah.mobi/app'");
    lines.push("");
